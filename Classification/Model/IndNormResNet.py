...
class IndNormResNet(NeuralNet):
    ...
    def __init__(self,
        ...
        if type(commun_block) is not list:
            shared_blocks = [self.__get_block(commun_block) for _ in range(split_level - 1)]
        else:
        ...
        for task, blocks in list(task_block.items()):
            if type(blocks) is not list:
                task_block_list[task] = [self.__get_block(blocks) for _ in range(5 - split_level)]
            else:
        ...
        self.first_conv = Convolution(dimensions=NB_DIMENSIONS,
                                      in_channels=num_in_chan,
                                      out_channels=self.__in_channels,
                                      kernel_size=first_kernel,
                                      act=act,
                                      norm=norm,
                                      conv_only=shared_blocks[0] in [IndPreResBlock])
    ...
    def forward(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        for task in self.__tasks:
            temp = self.shared_layers[0](task=task, x=out)
            for count in range(1, len(self.shared_layers)):
                temp = self.shared_layers[count](task=task, x=temp)
